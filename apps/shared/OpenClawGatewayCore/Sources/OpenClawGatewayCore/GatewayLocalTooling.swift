...
enum GatewayLocalTooling {
        ...
        urlSession: URLSession) async -> ToolResult
    {
        ...
        do {
            ...
            let timeoutSeconds = max(1.0, Double(toolParams.timeoutMs ?? 5_000) / 1000.0)
        ...
        urlSession: URLSession) async -> ToolResult
    {
        ...
        do {
            ...
            let timeoutSeconds = max(1.0, Double(toolParams.timeoutMs ?? 8_000) / 1_000.0)
            ...
            let maxChars = max(512, min(toolParams.maxChars ?? 24_000, 400_000))
        ...
        urlSession: URLSession) async -> ToolResult
    {
        ...
        let maxChars = max(512, min(toolParams.maxChars ?? 40_000, 500_000))
        let includeLinks = toolParams.includeLinks ?? true
        let timeoutMs = max(1_000, min(toolParams.timeoutMs ?? 20_000, 120_000))
        ...
        if let upstreamForwarder, let requestURL {
            ...
            if let waitUntil = toolParams.waitUntil?.trimmingCharacters(in: .whitespacesAndNewlines), !waitUntil.isEmpty {
        ...
        urlSession: URLSession) async -> ToolResult
    {
        ...
        let includeLinks = toolParams.includeLinks ?? true
        let maxChars = max(512, min(toolParams.maxChars ?? 20_000, 400_000))
    ...
    private static func looksLikeHTML(_ text: String) -> Bool {
        let sample = text.prefix(2_048).lowercased()
    ...
    private static func extractNormalizedContent(
        ...
        baseURL: URL?,
        includeLinks: Bool,
        maxChars: Int) -> (title: String?, text: String, originalChars: Int, truncated: Bool, links: [(href: String, text: String)])
    {
    ...
    private static func extractHTMLLinks(_ html: String, baseURL: URL?) -> [(href: String, text: String)] {
        ...
        for match in matches {
            ...
            if let url = URL(string: rawHref, relativeTo: baseURL)?.absoluteURL {
                resolvedHref = url.absoluteString
            } else {
        ...
        workspaceRoot: URL) -> ToolResult
    {
        ...
        do {
            ...
            let maxChars = max(1, min(toolParams.maxChars ?? 24_000, 200_000))
